...
\section{Giới thiệu}
    ...
    \end{equation}
    là đã được biết đến rộng rãi, ít nhất là từ công trình của Klein về khối hai mươi mặt \cite{klein}; xem Coxeter \cite[tr.~53]{coxeter1973} để có một cách trình bày hiện đại, đầy đủ. Có hai lớp vô hạn: 
    \begin{itemize}
    ...
    \end{align}
    sao cho $p$ chọn một đại diện duy nhất cho mỗi lớp tương đương trong $\so(3)/\sim$. Lưu ý ảnh bằng $\overline{B}$, được hiển thị trong Hình \ref{fig:log-so3}.  \emph{Mọi hình trong bài báo này là một hình chiếu của $\overline{B}$ lên một mặt phẳng. }
    \begin{remark}
        $\overline{B}$ chính là hình ảnh quen thuộc của $SO(3)$ như một khối cầu đặc bán kính $\pi$ với các điểm biên đối cực được đồng nhất, hiện thực hóa $SO(3)$ như không gian xạ ảnh thực $\R P^3$; xem Stillwell \cite{stillwell2008}.
    \end{remark}
    Mỗi lớp tương đương $[\hat{r}]\in\so(3)/\sim$ trong sơ đồ giao hoán: 
    ...
    \begin{equation}
        ...
        \end{tikzcd}
    \end{equation}
    Chúng ta tập trung vào $\overline{B}$ vì nó là một $SO(3)$-torsor dưới tác động sau: tác động nhân trái của $SO(3)$ lên chính nó là tự do và bắc cầu, và tác động dưới đây chính là tác động đó được chuyển qua song ánh $\log$ (với nghịch đảo $\exp|_{\overline{B}}$), nên nó thừa hưởng tính tự do và bắc cầu.
    \begin{align}
...
\end{gather}
 Tính chất \ref{pbd} là theo định nghĩa để xây dựng một phức ô hữu hạn.  Tính chất \ref{cinv} phát biểu rằng nhóm phải ổn định độc lập mỗi loại ô. Cuối cùng tính chất \ref{eucha} là đặc số Euler của các khối đa diện đồng phôi với mặt cầu \cite{MR1867354}.
...
%  bằng cách theo dõi tác động nhóm trên $P^i$ riêng biệt. 
 ...
 Tuy nhiên $P$ là đều đặn theo nghĩa cổ điển (đối xứng bắc cầu trên cờ, ví dụ một khối Platon) chỉ khi mọi 2-ô đều tương tự \emph{và} mọi 0-ô, 1-ô cũng tương tự; tính tương tự của riêng các 2-ô là cần nhưng chưa đủ, vì tính bắc cầu trên mặt không suy ra tính bắc cầu trên đỉnh hay cạnh (ví dụ một lưỡng chóp $n$-giác có thể có $D_n$ tác động bắc cầu trên $2n$ mặt tam giác mà không bắc cầu trên hai loại đỉnh của nó) \cite{coxeter1973}.
...
\end{align}
\emph{Định lý quỹ đạo-bộ ổn định} \cite{dummitfoote} cho chúng ta biết rằng: 
\begin{equation}
...
Cuối cùng, 6 2-chu trình cố định các đường chéo qua các trung điểm đối diện của các cạnh hình lập phương.  Khối đa diện được tạo thành bởi các điểm này là một khối lập phương bát diện. Các đại diện phải nằm trong một mặt cầu bán kính $\pi$ nên chúng cũng nằm trên $\partial\overline{B}$.
...
\begin{itemize}
    ...
    \item 5-chu trình cố định các đỉnh của icosahedron đối ngẫu và có bậc 5, vì vậy các đại diện của chúng là một \emph{cặp} icosahedra đối ngẫu, một bán kính $2\pi/5$ và một $4\pi/5$.
...
\section{Kết luận}
Hình học của chính các nhóm hữu hạn có liên quan mật thiết đến hình học của các đối tượng mà các đối xứng của chúng là nhóm đó:  bậc phần tử và lớp liên hợp xác định vị trí của các phần tử trong làm phẳng của chúng ta, cũng như cấu trúc nhóm con và lý thuyết quỹ đạo. Chúng ta đã thấy cách các khối đa diện chính quy và bán chính quy xuất hiện như các lớp liên hợp của các nhóm con hữu hạn của $SO(3)$ khi được nhúng vào đại số Lie $\so(3)$ thông qua ánh xạ logarit. 

Các trực quan hóa này cung cấp một cách hiểu trực quan về cấu trúc của các nhóm này và mối quan hệ của chúng với hình học không gian ba chiều. Biểu diễn trục-góc và việc xây dựng không gian thương $\overline{B}$ cho phép chúng ta "thấy" toàn bộ nhóm cùng một lúc, với các phần tử được sắp xếp theo cách phản ánh cả cấu trúc đại số của chúng (qua bậc và lớp liên hợp) và ý nghĩa hình học của chúng (qua các trục quay và góc).

Khái niệm về khối đa diện dẫn xuất mở rộng ý tưởng về tính đối ngẫu và cho thấy cách các nhóm con hữu hạn của $SO(3)$ liên quan đến toàn bộ họ các khối đa diện, không chỉ các khối đa diện đều. Điều này gợi ý rằng có một lý thuyết phong phú hơn về mối quan hệ giữa các nhóm và hình học chờ được khám phá, đặc biệt là trong các chiều cao hơn. 
